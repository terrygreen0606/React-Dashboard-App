...
class Quoteview extends Component {
    ...
    this.props.dispatch(QuoteEntryService.getQuoteSavedData(poTransPK, this.state.product))
      .then((res) => {
        this.setState({
          ...
          maillingAddrPK: res.applicantData.getMailingAddr.n_PersonAddressesId_PK || '',
  ...
  render() {
    return (
      <LoadingOverlay>
        <Helmet>
          <title>Avatar Insurance - Quote Elements HO3 Select</title>
        </Helmet>
        ...
        <Form method="post" onSubmit={this.saveQuoteEntryData} onKeyDown={(e) => { if (e.keyCode === 13) { e.preventDefault(); return false; } }} className="animated fadeIn mt-1">
          <Row>
            <Col xs="12">
              <strong><span className="badge badge-success"><h5><i className="fa fa-home"></i>&nbsp;&nbsp;Elements HO3 Select</h5></span></strong>
