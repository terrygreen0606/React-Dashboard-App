...
class Quoteview extends Component {
  ...
  setWhichButton = (e) => {
    this.setState({ whichButton: e.currentTarget.name, visible: false, successMsg: '' });
  }
  ...
  render() {
    return (
      <LoadingOverlay>
        <Helmet>
          <title>Avatar Insurance - Quote Elements HO6 Select</title>
        </Helmet>
        ...
        <Form method="post" onSubmit={this.saveQuoteEntryData} onKeyDown={(e)=>{if(e.keyCode === 13){e.preventDefault();return false;}}} className="animated fadeIn mt-1">
          <Row>
            <Col xs="12">
              <strong><span className="badge badge-success"><h5><i className="fa fa-home"></i>&nbsp;&nbsp;Elements HO6 Select</h5></span></strong>
              <Button type="submit" size="sm" color="success" name="Ratesubmit" className="pull-right mr-1" onClick={this.setWhichButton}><i className="fa fa-check"></i>&nbsp;&nbsp; Rate</Button>&nbsp;&nbsp;
              ...
              <Button type="submit" color="info" name="Ratesubmit" className="ratebt" onClick={this.setWhichButton} disabled={this.props.isLoading}><i className="fa fa-check"></i>&nbsp;&nbsp;&nbsp;Rate</Button>
          </Col>
          </Row>
          <hr />
          <Row>
