...
const EndorsmentCvg = props => {
  const [hdnEndorsCvg, sethdnEndorsCvg] = useState([11, 12, 106]);
  ...
  const [cvgADwelling, setCvgADwelling] = useState('');
  ...
  const setSelectedDDVal = () => {
    ...
    for (const [i, val] of props.quoteSavedData.getCvgData.entries()) {
      if (val.n_InsuredValueCoveredRate != 0 && !([37, 4, 5, 6].indexOf(val.n_PCCoverageID_PK) > -1)) {
    ...
    for (const [i, val] of props.quoteSavedData.getCvgData.entries()) {
      if (val.n_InsuredValueCoveredRate != 0 && !([37, 4, 5, 6].indexOf(val.n_PCCoverageID_PK) > -1)) {
        ...
        if(val.s_CoverageCode == 'HOMCVGSPA'){
          setCvgADwelling(val.n_InsuredValue);
        }
      }
    }
      ...
      {props.baseData != null ?
        props.baseData.all_endorsement_Coverage.map((cvg, i) => {
          ...
          if (cvg.s_LimitCodeType == 'NOEDIT') {
            ...
            fieldsPart = (
              <Col md="5"><Input type="text" bsSize="sm" name={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} id={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} value={cvgADwelling} onChange={(e)=>setCvgADwelling(e.currentTarget.value)} disabled={props.mainState.isBind}/></Col>
            );
