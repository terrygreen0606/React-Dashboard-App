...
import PropertyZone from '../Common/PropertyZone';
...
const ApplicantInfo = props => {
  ...
  const [propZoneHide, setPropZoneHide] = useState(true);
  const [propZoneData, setPropZoneData] = useState([]);
  ...
  const selAllSavedData = () => {
    ...
    if (applicantData.propertyZoneShow == 'Y') {
      setPropZoneHide(false);
    }
    setPropZoneData(applicantData.getRatingZone || []);
    setPropZone(props.quoteSavedData.getPoRiskAddiData.s_PremiuimGroup || '');
    setSavedData(true);
  ...
  const getDataFromZip = (zipCodeVal) => {
    ...
    props.dispatch(QuoteEntryService.getZipDetails(props.mainState.product, zipCodeVal, 'APPLICANT', d_TermStartDate))
      .then((res) => {
        if (res.status == 'N') {
          ...
          var tempArray = [...propZoneData];
          tempArray = res.ratingZone;
          setPropZoneData(tempArray);
        }
      });
  }
    ...
    <React.Fragment>
      ...
      <Row className="p-0 m-0">
        ...
        <Col md="1" className='p-1'>
            ...
            <Input type="text" bsSize="sm" name="data[TbPersonaddress][s_PostalCode]" id="s_PostalCode" placeholder="Enter Zip Code" value={zipCode} onChange={(e) => setZipCode(e.target.value)} onBlur={(e) => getDataFromZip(e.target.value)} onInput={(e) => onInputChange(e)} disabled={props.mainState.isBind} />
            <Input type="hidden" name="data[TbPersonaddress][n_Zipcodes_PK]" id="n_Zipcodes_PK" value={zipCodePK} />
        </Col>
        <Col md="2" className='p-1'>
      ...
      <Row className="mb-2" hidden={propZoneHide}>
        <Col md="12">
          <PropertyZone zoneData={propZoneData} selectedPropZone={propZone} disabled={props.mainState.isBind} />
        </Col>
      </Row>
      <Row className="p-0 m-0">
