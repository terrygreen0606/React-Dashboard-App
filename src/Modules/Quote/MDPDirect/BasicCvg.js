...
const BasicCvg = props => {
  ...
  return (
    <React.Fragment>
        ...
        props.baseData.all_basic_Coverage.map((cvg, i) => {
          ...
          if (cvg.s_LimitCodeType == 'NUMBER') {
            ...
            if (cvg.s_CoverageCode == 'HOMCVGD') {
              ...
              optionPart = (<React.Fragment><option value="10">{"10%"}</option></React.Fragment>);
            }
          ...
          return (
            <Row key={i} className="mt-1">
              <Col md="5">
                <Label className="m-0">{cvg.s_CoverageCode == 'HOMCVGD'?'Fair Rental Value':cvg.s_ScreenName}</Label>
              </Col>
              {fieldsPart}
            </Row>
          );
      ...
      <Input type="hidden" name="hdnService_BasciCVG" value={hdnBasicCvg.join('|')} />
    </React.Fragment>
  );
}
