...
import { customStyles } from '../../../utilities/reactSelectCustomStyle';
...
const QuoteEntryHeader = props => {
  ...
  const [isSetBindDate, isSetBindDateFun] = useState(false);
  ...
  useEffect(()=>{
    ...
    if(props.bindDate != '' && props.isBind && !isSetBindDate){
      setBinderDate(new Date(Moment(props.bindDate))); // setting binder date
      isSetBindDateFun(true); // preventing multiple update
    }
  });
      ...
      <Row className="p-0 m-0">
        <Col md="2">
          <FormGroup className="m-0">
            ...
            <Input type="text" bsSize="sm" name="QuoteNo" id="QuoteNo" placeholder="Quote No" value={props.policyNumber} readOnly disabled={props.isBind}/>
          </FormGroup>
        </Col>
        <Col md="2">
          <FormGroup className="m-0">
            <InputGroup className="input-prepend">
              <Label id='d_TermStartDate' className="mb-0"><i className="cui-calendar"></i>&nbsp;Effective From</Label>
            </InputGroup>
            <DatePicker 
              customInput={
              ...
              className="form-control form-control-sm" name="data[TbPotermmaster][d_TermStartDate]" selected={d_TermStartDate} onChange={(date) => onchangeTermStartDate(date)} disabled={props.isBind} required/>
          </FormGroup>
        </Col>
        <Col md="2">
          <FormGroup className="m-0">
            <InputGroup className="input-prepend">
              <Label id='d_TermStartDate' className="mb-0"><i className="cui-calendar"></i>&nbsp;Effective To</Label>
            </InputGroup>
            <DatePicker className="form-control form-control-sm" name="data[TbPotermmaster][d_TermEndDate]" selected={d_TermEndDate} readOnly disabled={props.isBind} />
          </FormGroup>
        </Col>
        <Col md="2">
          <FormGroup className="m-0">
            ...
            <DatePicker className="form-control form-control-sm" name="BinderDate" placeholderText="MM/DD/YYYY" selected={binderDate} readOnly disabled={props.isBind} />
          </FormGroup>
        </Col>
        <Col md="2">
          <FormGroup className="m-0">
            <InputGroup className="input-prepend">
              <Label id='FinalPremium' className="mb-0">Estimated Premium</Label>
            </InputGroup>
            <Input type="text" bsSize="sm" name="FinalPremium" id="FinalPremium" placeholder="Estimated Premium" value={props.estmPrem} readOnly disabled={props.isBind}/>
          </FormGroup>
        </Col>
        <Col md="2" className=''>
          <FormGroup className="m-0">
