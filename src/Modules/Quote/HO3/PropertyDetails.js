...
const PropertyDetails = props => {
  ...
  const setAllSavedData = () => {
    ...
    setRoofSubType(propertyData.s_RoofCondition || '');
  ...
  return (
    <React.Fragment>
      <Row className="p-0 m-0">
        <Col md="2" className="p-1">
          ...
          <InputGroup className="input-prepend">
            ...
            <Button type="button" size="sm" color="warning" disabled={props.mainState.isBind}>Get RCV</Button>
      ...
      <Row className="p-0 m-0">
        ...
        <Col md={widthSize} className='p-1'>
        ...
        <Col md={widthSize} className='p-1' hidden={hideRoofSubType}>
          <Label className="mb-0" id='ROOFSUBTYPE'>Sub-type {CreateTooltips('ROOFSUBTYPE', 'Roof sub-type')}</Label>
          <Input type="select" bsSize="sm" name="data[ROOFSUBTYPE]" value={roofSubType} onChange={(e) => setRoofSubType(e.target.value)} disabled={props.mainState.isBind}>
            <option key="" value="">Select</option>
            {props.baseData != null ? props.baseData.ROOFSUBTYPE.map((comp, i) => {
              return (
                <option key={i} value={comp.s_AppCodeName}>{comp.s_AppCodeNameForDisplay}</option>
              );
            }) : null}
