...
const EndorsmentCvg = props => {
  const [hdnEndorsCvg, sethdnEndorsCvg] = useState([11, 12]);
  ...
  const [WATERBACKP, setWATERBACKP] = useState('');
  const [EPICCOMPCVG, setEPICCOMPCVG] = useState('');
  const [HOBUSIPROPERTY, setHOBUSIPROPERTY] = useState('');
  ...
  const setSelectedDDVal = () => {
    ...
    for (const [i, val] of props.quoteSavedData.getCvgData.entries()) {
      if (val.n_InsuredValueCoveredRate != 0 && !([1, 2, 3, 4, 5, 6].indexOf(val.n_PCCoverageID_PK) > -1)) {
        ...
        } else if (val.s_CoverageCode == 'WATERBACKP') {
          setWATERBACKP(String(val.n_PRLimitFK));
        } else if (val.s_CoverageCode == 'EPICCOMPCVG') {
          setEPICCOMPCVG(String(val.n_PRLimitFK));
        } else if (val.s_CoverageCode == 'HOBUSIPROPERTY') {
          setHOBUSIPROPERTY(String(val.n_PRLimitFK));
        }
      }
    }
  ...
  const onChangedSelectedDDValue = (e) => {
    ...
    } else if (e.currentTarget.id == 'WATERBACKP') {
      setWATERBACKP(e.currentTarget.value);
    } else if (e.currentTarget.id == 'EPICCOMPCVG') {
      setEPICCOMPCVG(e.currentTarget.value);
    } else if (e.currentTarget.id == 'HOBUSIPROPERTY') {
      setHOBUSIPROPERTY(e.currentTarget.value);
    }
  }
      ...
      {props.baseData != null ?
        props.baseData.all_endorsement_Coverage.map((cvg, i) => {
          ...
          if (cvg.s_LimitCodeType == 'NOEDIT') {
            fieldsPart = (
              <React.Fragment>
                <Col md="5"><Input type="hidden" bsSize="sm" name={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} id={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} readOnly disabled={true} /></Col>
              </React.Fragment>
            );
          } else if (cvg.s_LimitCodeType == 'NUMBER') {
            fieldsPart = (
              <Col md="5"><Input type="hidden" bsSize="sm" name={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} id={"txtCvgLimitAmt_" + cvg.n_PCCoverageID_PK} disabled={true} /></Col>
            );
            ...
            fieldsPart = (
              <React.Fragment>
                ...
                <Col md="2" className="p-1">{formLink}</Col>
              </React.Fragment>
            );
          } else {
            var defaultSelected = {};
            ...
            } else if (cvg.s_CoverageCode == 'WATERBACKP') {
              defaultSelected['value'] = WATERBACKP;
            } else if (cvg.s_CoverageCode == 'EPICCOMPCVG') {
              defaultSelected['value'] = EPICCOMPCVG;
            } else if (cvg.s_CoverageCode == 'HOBUSIPROPERTY') {
              defaultSelected['value'] = HOBUSIPROPERTY;
            }
