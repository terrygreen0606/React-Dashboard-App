...
const UWQuestions = props => {
  ...
  const setSelectedVal = () => {
    ...
    for (const [i, val] of props.quoteSavedData.getUWQesAns.entries()) {
      if (val.s_PoAdditionalStatsAnswer == 'Yes' && ([12890171, 12984293, 12890194, 12890221].indexOf(val.n_PRPolicyAddStatQuestions_FK) > -1)) {
        savedArray.push(parseInt(val.n_PRPolicyAddStatQuestions_FK)) // Pushed into array is ans 'Yes' and qus pk from [12890171, 12984293, 12890194, 12890221]
      }
    }
