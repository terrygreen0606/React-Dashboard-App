...
const EndorsmentCvg = props => {
  ...
  const [PPDECRLMT, setPPDECRLMT] = useState('');
  const [HOMCVGBADD, setHOMCVGBADD] = useState('');
  const [HOMCVGCADD, setHOMCVGCADD] = useState('');
  ...
  const setSelectedDDVal = () => {
    ...
    for (const [i, val] of props.quoteSavedData.getCvgData.entries()) {
      if (val.n_InsuredValueCoveredRate != 0 && !([1, 2, 3, 4, 5, 6, 7].indexOf(val.n_PCCoverageID_PK) > -1)) {
    ...
    for (const [i, val] of props.quoteSavedData.getCvgData.entries()) {
      if (val.n_InsuredValueCoveredRate != 0 && !([1, 2, 3, 4, 5, 6, 7].indexOf(val.n_PCCoverageID_PK) > -1)) {
        ...
        } else if (val.s_CoverageCode == 'PPDECRLMT') {
          setPPDECRLMT(String(val.n_PRLimitFK));
        } else if (val.s_CoverageCode == 'HOMCVGBADD') {
          setHOMCVGBADD(String(val.n_PRLimitFK));
        } else if (val.s_CoverageCode == 'HOMCVGCADD') {
          setHOMCVGCADD(String(val.n_PRLimitFK));
  ...
  const onChangedSelectedDDValue = (e) => {
    ...
    } else if (e.currentTarget.id == 'PPDECRLMT') {
      setPPDECRLMT(e.currentTarget.value);
    } else if (e.currentTarget.id == 'HOMCVGBADD') {
      setHOMCVGBADD(e.currentTarget.value);
    } else if (e.currentTarget.id == 'HOMCVGCADD') {
      setHOMCVGCADD(e.currentTarget.value);
      ...
      {props.baseData != null ?
        props.baseData.all_endorsement_Coverage.map((cvg, i) => {
          ...
          if (cvg.s_LimitCodeType == 'NOEDIT') {
            ...
            } else if (cvg.s_CoverageCode == 'PPDECRLMT') {
              defaultSelected['value'] = PPDECRLMT;
            } else if (cvg.s_CoverageCode == 'HOMCVGBADD') {
              defaultSelected['value'] = HOMCVGBADD;
            } else if (cvg.s_CoverageCode == 'HOMCVGCADD') {
              defaultSelected['value'] = HOMCVGCADD;
